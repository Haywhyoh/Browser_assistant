...
export default function EditNote() {
  ...
  return (
      ...
      <form onSubmit={handleSubmit}>
        <label>Title <br></br>
        <input type="text" defaultValue={title} onChange={(e) => setTitle(e.target.value)} id="note_title"/>
        </label> <br></br>
        <label>Write Note: 
        <textarea defaultValue={content} onChange={(e) => setNote(e.target.value)} cols={40} rows={15}  id='note_value'/>
        </label>
        <Link to={'/organizer'}><input type="submit" value="Submit" id='set_note' onClick={() => {
             const title = document.querySelector('#note_title').defaultValue;
             const content = document.querySelector('#note_value').defaultValue;
